...
<body>
    ...
    <section id="templatePage" style="display: none; opacity: 0; transition: opacity 0.5s ease;">
        ...
        <div class="slider-container">
            ...
            <div class="slider" id="slider">
                @foreach($frames as $frame)
                    <div class="slide">
                        <img src="{{ $frame->path }}" width="100%" height="auto" 
        ...
        <a href="#" id="select-frame-button">Click to Start</a>
    </section>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            let selectedFrameId = null; // Store the selected frame ID
            const frameOptions = document.querySelectorAll(".frame-option");
            const selectButton = document.getElementById("select-frame-button");

            // Add click event to each frame
            frameOptions.forEach(frame => {
                frame.addEventListener("click", function () {
                    selectedFrameId = this.getAttribute("data-id");

                    // Highlight the selected frame (optional)
                    frameOptions.forEach(f => f.style.border = "2px solid #000"); // Reset others
                    this.style.border = "2px solid blue"; // Highlight selected
                });
            });

            // Update the button URL when clicked
            selectButton.addEventListener("click", function (event) {
                if (selectedFrameId) {
                    this.href = `/cam?frameId=${selectedFrameId}`;
                } else {
                    event.preventDefault(); // Prevent navigation if no frame is selected
